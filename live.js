...
var markercount = (local.parameters.numberOfMarkers.get())*2 ;
var marks ;
var id = 0 ;
var col = 0xFF000000;
var nam = "";
...
//		clip.addTrigger("Sync Clip Labels", "Get Clip Labels and Infos Feedback from Live Session" , false);
...
//Markers Container >>>>>>>>>>>>>>>>>>>>>>		
		...
		mark.addTrigger("Reset Markers", "Reset All Labels" , false);	
...
function moduleParameterChanged(param) {
		...
		local.parameters.numberOfTracks.set(8);
  		...
  		local.parameters.numberOfMarkers.set(6);}
...
// >>>>> Show ColorID Name and Hex-Color
	if (value.name == "colorId"){
		id = local.values.infos.colorId.get()  ;
		col = livecolors[id].hex;
		nam = livecolors[id].name;
...
// >>>>> Sync Scenes and Tracks Number  	
  	if (value.name == "updateChataigneSettings"){
  		...
  		var marknu = local.values.infos.allMarkers.get();
...
// >>>>> Reset Markers 	  	
  	if (value.name == "resetMarkers"){ 
  		for (var n = 0; n < markercount/2; n++) {
  		var no = n +1 ;
		local.values.markers.getChild("marker"+no).set("") ;
		local.values.markers.getChild("beatM"+no).set(0) ;
		} }
...
// >>> Song is Playing ??
 	if (address == "/live/song/get/is_playing") {
 		...
 		local.values.songIsPlaying.set(args[0]);}	
...
// >>> insert Marker Labels	
	if (address == "/live/song/get/cue_points") {
		markercount = args.length ;
		marks = markercount / 2 ;
		local.parameters.numberOfMarkers.set(marks) ;
		local.values.infos.allMarkers.set(marks) ;
		for (var n = 0; n < markercount; n++) {
		var no = Math.round((n + 2)/2) ;
		var mark = args[n];
		var beatm = args[n+1];
		local.values.markers.getChild("marker"+no).set(mark) ;
		local.values.markers.getChild("beatM"+no).set(beatm) ;
		n= n + 1 ;
		}  }
